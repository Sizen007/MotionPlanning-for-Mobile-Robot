...
for i=0:n_seg-1
    ...
    Pyi = flipud(poly_coef_y(i*(n_order+1)+1:(i+1)*(n_order+1)));    
...
function poly_coef = MinimumSnapCloseformSolver(waypoints, ts, n_seg, n_order)
    ...
    %
    %
    dF=[start_cond(1); start_cond(2); start_cond(3); start_cond(4);];
    for n = waypoints(2:end-1)
        dF = [dF; n]
    end
    dF=[dF; end_cond(1); end_cond(2); end_cond(3); end_cond(4);];    
    dP = -inv(R_pp) * R_fp' * dF;
