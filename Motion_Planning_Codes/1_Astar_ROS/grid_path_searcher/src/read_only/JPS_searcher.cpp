...
//jieming
inline int isInClosed(GridNodePtr *** GridNodeMap, const Eigen::Vector3i& temp_coord) 
{
    int flag = 0;
...
inline int isInOpen(GridNodePtr *** GridNodeMap, const Eigen::Vector3i& temp_coord) 
{
    int flag = 0;
    //if(GridNodeMap[temp_coord(0)][temp_coord(1)][temp_coord(2)]->gScore < LONG_MAX){
    if(GridNodeMap[temp_coord(0)][temp_coord(1)][temp_coord(2)]->id ==0){
        flag = 1;
    }
    return flag;  
}
        ...
        */         
        for(int i = 0; i < (int)neighborPtrSets.size(); i++){
            ...
            *
            ...
            IMPORTANT NOTE!!!
            neighborPtrSets[i]->id = -1 : unexpanded new point
            ...
            */
            ...
            auto temp_coord = neighborPtr->index;

            if(isInClosed(GridNodeMap, temp_coord)){
                continue;
            }
            else if(isInOpen(GridNodeMap,temp_coord)){
                neighborPtr -> id  = 0;
            }
            else{ // new point
                neighborPtr -> id  = 1;
